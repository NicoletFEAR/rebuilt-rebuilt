 ...
 * converted into a CTRE {@link CANrangeConfiguration} using
 * {@link #getCanRangeConfiguration()}.
 *
 * @param id              CAN ID and CAN bus of the CANrange sensor
 * @param sensorTolerance the distance at which the CANrange starts sensing
 *                        distance
 */
public record CanRangeConfig(CanId id, Distance sensorTolerance) {
    ...
    public CANrangeConfiguration getCanRangeConfiguration() {
        return new CANrangeConfiguration()
                .withProximityParams(
                        new ProximityParamsConfigs().withProximityThreshold(sensorTolerance));
    }
}
